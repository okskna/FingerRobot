...
#include "Pro_main.hpp"
#include "Pro_roi.hpp"
#include "Pro_gesture.hpp"
#include <vector>
#include <cmath>
...
int main(){
	...
	if (!serialComm.connect("COM7")) //COM4번의 포트를 오픈한다. 실패할 경우 -1을 반환한다.
	{
    ...
    for(;;){
        ...
        //finger Number all sec.
        cout <<"finger number : " << hg.mostFrequentFingerNumber << endl;
		...
		*/
		...
		if (hg.mostFrequentFingerNumber >= 0 && hg.mostFrequentFingerNumber <= 5)
		{
			printf("\nA");
			buffer = (char)hg.mostFrequentFingerNumber + '0';
			printf("%c", buffer);
