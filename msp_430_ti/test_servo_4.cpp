...
int main(void)
{
    ...
    P1DIR |= BIT6 + BIT2;							// P1.6/TA0.1 is used for PWM, thus also an output -> servo 1
    P2DIR |= BIT2 + BIT1 + BIT4 + BIT5;							// P2.2/TA1.1 is used for PWM, thus also an output -> servo 2

	P1OUT = 0; 								// Clear all outputs P1
	P2OUT = 0; 								// Clear all outputs P2

  	P1SEL |= BIT6 + BIT2;                          // P1.6 select TA0.1 option
  	P2SEL |= BIT2 + BIT1 + BIT4 + BIT5;                          // P2.2 select TA1.1 option
...
int main(void)
{
    ...
    P1DIR |= BIT6 + BIT2;							// P1.6/TA0.1 is used for PWM, thus also an output -> servo 1
    P2DIR |= BIT2 + BIT1 + BIT4 + BIT5 + BIT6;							// P2.2/TA1.1 is used for PWM, thus also an output -> servo 2

	P1OUT = 0; 								// Clear all outputs P1
	P2OUT = 0; 								// Clear all outputs P2

  	P1SEL |= BIT6 + BIT2;                          // P1.6 select TA0.1 option
  	P2SEL |= BIT2 + BIT1 + BIT4 + BIT5 + BIT6;                          // P2.2 select TA1.1 option
